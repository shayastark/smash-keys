...
- **Play mode** — **Classic** is the original experience. **Jam** adds a steady drum pattern in the background; **beat tempo** and **beat strength** sliders show only when Jam is selected (default **140 BPM**, range **70–280**, i.e. half to double that default). **Groove** uses softer, sweeter melodic sounds for keys (still no rules or scoring).
